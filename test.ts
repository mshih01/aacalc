...
function remove_planehits3( node : naval_unit_graph_node , hits : number) : number
{
	let n = hits;
	return node.nairArr[n];	
}

function remove_dlast_planehits3( node : naval_unit_graph_node , hits : number) : number
{
	let n = hits;
...
function solve_one_naval_state(problem : naval_problem, N : number, M : number, allow_same_state : boolean, numBombard : number)
{
	...
	let def_remove_planehits_function = remove_dlast_planehits3;
	let def_remove_navalhits_function = remove_dlast_navalhits2;
	if (!att_destroyer) {
		def_remove_planehits_function = remove_dlast_navalhits2;

	}
	if (!def_dlast) {
		def_remove_subhits_function = remove_dlast_subhits2;
		def_remove_planehits_function = remove_dlast_planehits3;
		if (!att_destroyer) {
			def_remove_planehits_function = remove_dlast_navalhits2;
		}
	...
	let att_remove_planehits_function = remove_dlast_planehits3;
	let att_remove_navalhits_function = remove_dlast_navalhits2;
	if ( !def_destroyer) {
	    att_remove_planehits_function = remove_dlast_navalhits2;
	}
	if (!att_dlast) {
		att_remove_subhits_function = remove_subhits2;
		att_remove_planehits_function = remove_planehits3;
		att_remove_navalhits_function = remove_navalhits2;
		if ( !def_destroyer) {
			att_remove_planehits_function = remove_navalhits2;
		}
	}
	problem.setP(N, M, 0);
        ...
        for (i1 = 0; i1 <= N1; i1++) {
            ...
            for (j1 = 0; j1 <= M1; j1++) {
                ...
                for (i2 = 0; i2 <= newN2; i2++) {
					...
					let m2 = def_remove_planehits_function(defnode2, i2);
                    ...
                    for (j2 = 0; j2 <= newM2; j2++) {
						...
						let n2 = att_remove_planehits_function(attnode2, j2);
